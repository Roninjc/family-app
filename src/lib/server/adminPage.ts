...
type AdminLoadContext = {
  locals: App.Locals
  cookies: Cookies
  url: URL
}

const resolveRequestedFamilyIdFromForm = (form: FormData, forcedFamilyId?: string | null) => {
  if (forcedFamilyId !== undefined) return forcedFamilyId
  return String(form.get('familyId') ?? '').trim() || null
}

export const loadAdminPage = async (
  { locals, cookies, url }: AdminLoadContext,
  options?: { requestedFamilyId?: string | null }
) => {
  const managerContext = await resolveManagerFamily({
    locals,
    cookies,
    requestedFamilyId: options?.requestedFamilyId ?? url.searchParams.get('family')
  })
  ...
  if (isMockFamilyMode()) {
    ...
    return {
      ...
      availableFamilies: managerContext.families,
      activeFamilyId: managerContext.activeFamily.id,
