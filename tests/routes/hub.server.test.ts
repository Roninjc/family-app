...
import { actions, load } from '../../src/routes/dashboard/+page.server'
...
describe('dashboard load', () => {
  it('builds family panels and sets active family cookie from query', async () => {
    ...
    const data = await (load as any)({
      ...
      cookies,
      url: new URL('http://localhost/dashboard?family=f2')
    })
  ...
  it('falls back to cookie family when query family is invalid', async () => {
    ...
    const data = await (load as any)({
      ...
      url: new URL('http://localhost/dashboard?family=missing')
    })
...
describe('dashboard notes actions', () => {
