...
- `yarn run check` — type-check with svelte-check (must be `yarn run check`; plain `yarn check` runs yarn's builtin integrity check instead). There is no test suite, so this is the main validation.
...
3. `src/stores/tree.ts` — the tree engine. `familyTree` is an adjacency-list graph (Map of node id → relations Map) with `Relation` enum weights (Child=1, Parent, Sibling, Partner, PreviousPartner); Child/Parent edges auto-insert their inverse. DFS traversals compute `generations`/`firstGeneration` (oldest generation = 1) and `parentsChildrenArray` (couples + common children). These are mutable `export let` bindings reassigned on rebuild. `visitedMembers`/`stack` writable stores coordinate the recursive render so each member renders once.
4. `src/components/treeNode.svelte` renders recursively (partner/siblings/children via `svelte:self`); `addFamilyMemberModal.svelte` is a 4-step form that POSTs to the `?/addMember` action (hidden inputs on the summary step, `use:enhance`, `invalidateAll` on success re-renders the tree).
5. `src/components/connectionLines.svelte` + `src/lib/utils/connectionLines.ts` draw SVG lines by measuring rendered DOM positions (member badge DOM ids are member uuids) after mount — layout must be complete (hence `setTimeout` before measuring/centering).
...
- Shared types live in `src/lib/types/familyTypes.d.ts` (beware existing typos like `PartnerRealtionInfo` that are referenced throughout).
