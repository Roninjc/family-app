...
4. `src/components/treeNode.svelte` renders recursively (partner/siblings/children via `svelte:self`); connection lines are only drawn to children rendered under that node (a child already rendered in another branch keeps its badge there). `addFamilyMemberModal.svelte` is a 4-step form that POSTs to the `?/addMember` action (hidden inputs on the summary step, `use:enhance`, `invalidateAll` on success re-renders the tree); it includes a "Hijos" field (existing members as children of the new one → RPC `parent_of` direction) so ancestors can be added, and the action also inserts a father↔mother `partner` edge if none exists, since children only get lines under a linked couple. Clicking a member badge opens `editMemberModal.svelte` (`editingMemberId`/`showEditMemberModal` stores): viewers see read-only details; editors can update name/family name/birth date (`?/updateMember`) or delete the member (`?/deleteMember`, relationships cascade via FK).
5. `src/components/connectionLines.svelte` + `src/lib/utils/connectionLines.ts` draw SVG lines by measuring rendered DOM positions (member badge DOM ids are member uuids) after mount — layout must be complete (hence `setTimeout` before measuring/centering). Previous partners with no common children get a dashed line. Window resize re-mounts the tree (debounced `treeVersion` bump in `+page.svelte`) to re-measure.
